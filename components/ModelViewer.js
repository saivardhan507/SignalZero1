 ...
 */
export default function ModelViewer({ src, alt, autoRotate = true, className = "", loadingAttr = 'lazy' }) {
  const modelRef = useRef(null);

  return (
    <div className={`relative w-full aspect-video min-h-[300px] md:min-h-[350px] lg:min-h-[450px] rounded-3xl overflow-hidden glass border border-white/5 shadow-2xl will-change-transform translate-z-0 ${className}`} style={{ transform: 'translateZ(0)', WebkitMaskImage: '-webkit-radial-gradient(white, black)' }}>
      <model-viewer
        ref={modelRef}
        src={src}
        alt={alt}
